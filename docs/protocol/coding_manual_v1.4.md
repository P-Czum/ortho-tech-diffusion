...
## v1.4 (English) · 2026-09-02

**This is a revision, not a translation.** v1.4 adds one material-eligibility rule (§4a) to v1.3; v1.3 changed one operational rule (§2, §4 step 2)
...
## 4a. Two- and three-letter abbreviations do not enter the material unverified

Added in v1.4. Forced by a fourth occurrence of the same failure: `ml` (millilitres, found and
removed in Stage 1), then `ha`, `cr`, `ka` and `ai`, each of which sat in or beside the material
while denoting several unrelated things.

> A term of two or three letters is admitted to the material only after its expansions have been
> measured on the titles and abstracts of the documents that contain it. It is admitted if a
> single expansion accounts for the clear majority of those documents; otherwise it is excluded,
> and the exclusion is reported with the measured share.

Measured shares that forced the rule:

| abbreviation | documents | dominant expansion |
|---|---:|---|
| `ha` | 960 | none — hydroxyapatite 17.7%, hyaluronic acid 14.3%, hip arthroplasty 28.6% |
| `cr` | 526 | cruciate retaining 21.9% |
| `ka` | 295 | kinematic alignment 31.5% |
| `ai` | 326 | artificial intelligence 41.1%, acetabular index 27.3% |
| `let` | 139 | lateral extra-articular tenodesis **93.5%** — admitted |

**Why exclusion does not discard recoverable information.** A group counts documents containing
*any* member, so a paper writing both "AI" and "artificial intelligence" is already counted by
the full form. The abbreviation contributes only documents using the abbreviation *alone* —
67.2% for `ai`, 76.6% for `ka`, 82.2% for `ha`, 92.6% for `cr`. What is discarded is precisely
the part that cannot be verified; what is verifiable is already in the group.
...
- **v1.4** (2026-09-02) — adds §4a, the abbreviation rule, after a fourth ambiguous
  abbreviation was found in or beside the material. Material eligibility only; no coding category,
  threshold or blinding rule changed. Carries a declared, unmeasured risk (§4a, last paragraph)
  that must be closed before the material is frozen.
